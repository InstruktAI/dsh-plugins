...
test('parseHookConfig rejects unimplemented fields, tolerates falsy, preserves once', () => {
  const mk = (field) => JSON.stringify({
    PreToolUse: [{ matcher: 'Read', hooks: [{ type: 'command', command: 'echo x', [field]: true }] }],
  })
  for (const field of ['async', 'asyncRewake', 'statusMessage']) {
    assert.throws(() => parseHookConfig(mk(field)), new RegExp(`hook field "${field}" is not implemented`))
  }
  // falsy declarations are tolerated (treated as absent); `once` stays honored
  const cfg = parseHookConfig(
    JSON.stringify({ PreToolUse: [{ matcher: 'Read', hooks: [{ type: 'command', command: 'echo x', once: true, async: false, asyncRewake: false, statusMessage: false }] }] }),
  )
  const h = cfg.get('PreToolUse')[0].hooks[0]
  assert.equal(h.once, true)
  assert.equal(h.async, false)
  assert.equal(h.asyncRewake, false)
})
