// dsh-hooks: Claude Code style hooks for DeepSeek Harness (fork, rc.7-verified).
//
...
//
// Event wiring (verified against rc.7 harness source, Aug 2026):
//   tools/pre-execute    -> PreToolUse    (deny materializes an official failed tool card)
//   tools/post-execute   -> PostToolUse / PostToolUseFailure (result.isError)
//   agent/pre-step       -> PostToolBatch (once per tool batch, before the next model call)
//   agent/inbox/inserted -> UserPromptSubmit (default top-level only)
//   agent/session-start  -> SessionStart
//   agent/turn-stopping  -> Stop          (stop_hook_active + last_assistant_message)
//   agent/error          -> StopFailure   (turn ended by an error)
//   subagent/start       -> SubagentStart (fires at the delegator, routed by parent)
//   subagent/end         -> SubagentEnd   (fires at the delegator, routed by parent)
//   agent/disposed       -> SessionEnd    (reason: 'exit') + cleanup
//
// CC base input fields (all events):
//   session_id, transcript_path, cwd, permission_mode, hook_event_name
//   - transcript_path  = DSH session JSONL artifact path (sessionPersistence.locate)
//   - permission_mode  = folded from session events:
//       plan mode active -> "plan"; sandbox danger-full-access -> "bypassPermissions";
//       approval never -> "acceptEdits"; otherwise "default"
//
// Trust model: hooks.json is executable project content, same trust as
// package.json scripts. Commands run through the shell service under the
// SESSION's sandbox policy by default; the explicit `"unsandboxed": true`
// opt-out (and http header env interpolation via `allowedEnvVars`) is honored
// only for hooks from the host-owned GLOBAL layer (~/.dsh/hooks.json).
//
...
// No hard service injects: the web console route is optional (registered only
// when a webServer service is present), so the plugin activates in headless
// and TUI profiles too.
...
// CC hook event names this build wires to harness events. Config entries for
// other CC event names (Notification, MessageDisplay, ConfigChange, ...) are
// accepted at parse time but have no harness counterpart and are skipped.
...
/**
 * Fold the CC `permission_mode` value for a session from its durable events:
 * plan mode active -> "plan"; sandbox "danger-full-access" -> "bypassPermissions";
 * approval "never" -> "acceptEdits"; otherwise "default".
 * Pure — mirrors the harness's own knob folds (dsh-plan-mode, dsh-sandbox-policy,
 * dsh-user-approval) so the hook input always matches what the session enforces.
 */
export function foldPermissionMode(events) {
  ...
  for (const event of events || []) {
    if (event.type === 'plan/mode') plan = event.data && event.data.active === true
    else if (event.type === 'sandbox/mode') sandbox = event.data && event.data.mode
    else if (event.type === 'approval/policy') approval = event.data && event.data.policy
  }
  if (plan) return 'plan'
...
/**
 * Text of the last assistant message in the session log (for Stop's
 * `last_assistant_message` and SubagentEnd's payload): the last
 * `assistant/message` event whose content carries at least one text block.
 */
export function lastAssistantMessageText(events) {
 ...
 *
 * Accepts BOTH Claude Code settings shapes:
 *   - nested:  { "$schema": ..., "hooks": { "SessionStart": [...] } }
 *   - flat:    { "SessionStart": [...] }
 * Unknown top-level keys ($schema, ...) are ignored.
 */
export function parseHookConfig(text) {
  ...
  const eventMap = json.hooks !== undefined ? json.hooks : json
  if (eventMap === null || typeof eventMap !== 'object' || Array.isArray(eventMap)) {
    throw new Error('hooks.json "hooks" must be an object of hook-event arrays')
  }
  ...
  for (const [event, matchers] of Object.entries(eventMap)) {
    ...
    for (const raw of matchers) {
      ...
      for (const hook of hooks) {
        ...
        // Contract discipline: fields this executor does not honor are rejected
        // at parse time instead of being silently documented-but-ignored.
        if (hook.shell !== undefined && hook.shell !== 'bash') {
          throw new Error(`event "${event}": unsupported shell "${String(hook.shell)}" (only "bash" is implemented)`)
        }
        for (const unsupported of ['async', 'asyncRewake', 'statusMessage']) {
          if (hook[unsupported] !== undefined && hook[unsupported] !== false) {
            throw new Error(`event "${event}": hook field "${unsupported}" is not implemented by dsh-hooks and must not be declared`)
          }
        }
      }
...
export async function loadConfig(presetDir, cwd, warn, globalHooksPath) {
  ...
  for (const layer of layerFiles(presetDir, cwd, globalHooksPath)) {
    ...
    for (const [event, matchers] of layerEvents) {
      // Stamp provenance: privilege elevation and env interpolation are only
      // honored for hooks from the host-owned GLOBAL layer (see executor).
      for (const group of matchers) {
        for (const hook of group.hooks) hook._layer = layer.kind
      }
...
function eventMatchQuery(hookEvent, hookInput) {
  switch (hookEvent) {
    ...
    default:
      return undefined // PostToolBatch etc. have no matcher support (matches all)
  }
}

// ---------------------------------------------------------------------------
// CC-compatible hook input construction
// ---------------------------------------------------------------------------

/**
 * Resolve the DSH session transcript artifact path for the session (the CC
 * `transcript_path` equivalent). Uses the jsonl persistence backend's
 * `locate()` so no filesystem touch is needed and the path is valid even
 * before the first durable flush.
 */
export function resolveTranscriptPath(persistence, session) {
  if (!session || !session.header) return undefined
  try {
    if (persistence && typeof persistence.locate === 'function') {
      const located = persistence.locate(session.header)
      if (located && typeof located.path === 'string' && located.path.length > 0) return located.path
    }
  } catch { /* best effort */ }
  return undefined
}
...
/**
 * Build the CC-compatible base hook input for one event. Base fields follow
 * CC's createBaseHookInput: session_id, transcript_path, cwd, permission_mode,
 * hook_event_name. Subagent-triggered hooks additionally carry agent_id /
 * agent_type / delegation_depth (DSH delegation fields).
 */
export function hookInputBase(ctx, agent, hookEvent) {
  ...
  return {
    ...
    transcript_path: resolveTranscriptPath(ctx ? ctx.get('sessionPersistence') : undefined, agent.session),
    cwd,
    permission_mode: foldPermissionMode(agent.session ? agent.session.events : undefined),
...
async function runCommandHook(ctx, state, hook, hookInput, signal) {
  ...
  // Hooks run under the SESSION's sandbox policy by default, so a project's
  // .dsh/hooks.json (possibly from a cloned repository) cannot escape the
  // agent's confinement. The `"unsandboxed": true` opt-out (the bash-sandbox
  // executor treats mode 'danger-full-access' as no confinement) is honored
  // ONLY for hooks from the host-owned GLOBAL layer (~/.dsh/hooks.json) —
  // provenance stamped at load time; project/preset hooks can never elevate.
  // TeleClaude's receiver hooks live in the global layer and declare it
  // because they legitimately write outside the workspace (e.g. ~/.local/state).
  let sandboxPolicy
  const sandboxSvc = ctx.get('sandboxPolicy')
  if (sandboxSvc && typeof sandboxSvc.resolve === 'function') {
    try {
      const base = state.agent ? { session: state.agent.session } : {}
      const trusted = hook._layer === 'global'
      sandboxPolicy =
        trusted && hook.unsandboxed === true
          ? sandboxSvc.resolve({ ...base, mode: 'danger-full-access' })
          : sandboxSvc.resolve(base)
    } catch (error) {
...
/**
 * Interpolate $VAR / ${VAR} references in http hook header values, but only
 * for variables explicitly listed in `allowedEnvVars` (unlisted references
 * resolve to the empty string).
 */
function interpolateHttpHeaders(headers, allowedEnvVars, trusted) {
  const out = {}
  // Env interpolation in header values is permitted only for host-owned
  // global-layer hooks; project/preset hooks get no interpolation at all.
  const allowed = new Set(trusted && Array.isArray(allowedEnvVars) ? allowedEnvVars : [])
  for (const [key, value] of Object.entries(headers || {})) {
    if (typeof value !== 'string') {
      out[key] = value
      continue
    }
    out[key] = value.replace(/\$\{([A-Za-z_][A-Za-z0-9_]*)\}|\$([A-Za-z_][A-Za-z0-9_]*)/g, (match, braced, plain) => {
      const name = braced !== undefined ? braced : plain
      return allowed.has(name) && process.env[name] !== undefined ? process.env[name] : ''
    })
  }
  return out
}
...
async function runHttpHook(state, hook, hookInput, signal) {
  ...
  try {
    ...
    const res = await fetch(hook.url, {
      method: 'POST',
      headers: { 'content-type': 'application/json', ...(interpolateHttpHeaders(hook.headers, hook.allowedEnvVars, hook._layer === 'global')) },
...
/** Does this result ask to continue the turn (Stop/UserPromptSubmit semantics)? */
function wantsContinue(r) {
  if (r.async || !r.json) return false
  if (r.json.decision === 'block') return true
  if (r.json.continue === false) return false // explicit hop-stop
  const specific = r.json.hookSpecificOutput
  return !!(specific && typeof specific.additionalContext === 'string' && specific.additionalContext.length > 0)
}

// A PreToolUse decision aggregate: { kind: 'allow'|'deny'|'ask', reason }.
...
function createController(ctx, agent, cwd) {
  return {
    ...
    pendingBatch: [], // tool calls of the current batch, for PostToolBatch
    stopContinued: false, // a Stop hook kept the current turn alive (stop_hook_active)
...
// ---------------------------------------------------------------------------
// event wiring (rc.7-verified names and payloads)
// ---------------------------------------------------------------------------
...
function wireAgent(ctx, agent) {
  ...
  agentCtx.on('tools/pre-execute', async (exec, next) => {
    ...
    try {
      const hookInput = {
        ...hookInputBase(ctx, agent, 'PreToolUse'),
        tool_name: exec.name,
  ...
  // additionalContext rides the DSH-native decision.additionalContexts channel
  // (ferried to the loop's active-batch FIFO — the closest analog to CC's
  // PostToolUse additionalContext). Also records the call into the pending
  // batch for PostToolBatch.
  agentCtx.on('tools/post-execute', async (exec, result, next) => {
    ...
    try {
      ...
      const tool_response = outputTextOf(result)
      const hookInput = {
        ...hookInputBase(ctx, agent, hookEvent),
        tool_name: exec.name,
        tool_input: exec.arguments ?? {},
        tool_use_id: String(exec.callId ?? ''),
      ...
      if (state.pendingBatch.length < BATCH_CAP) {
      ...
      log(ctx, 'error', `agent ${agent.id} ${result && result.isError ? 'PostToolUseFailure' : 'PostToolUse'} hooks failed: ${String(error)}`)
      return next()
    }
  })

  // PostToolBatch: fires once per tool batch, before the next model call —
  // hooked into the `agent/pre-step` waterfall. Only fires when the pending
  // batch is non-empty (CC: a turn with no tool calls produces no firing).
  agentCtx.on('agent/pre-step', async (payload, next) => {
    if (state.disposed || state.pendingBatch.length === 0) return next()
    const batch = state.pendingBatch
    state.pendingBatch = []
    try {
      const hookInput = {
        ...hookInputBase(ctx, agent, 'PostToolBatch'),
        ...(payload && typeof payload.turn === 'number' ? { turn: payload.turn } : {}),
        tool_calls: batch,
      }
      const results = await executeHooks(ctx, state, agent, 'PostToolBatch', hookInput, payload && payload.signal)
      const decision = await next()
      const context = collectAdditionalContext(results)
      if (context.length > 0 && decision && decision.kind !== 'reject' && Array.isArray(decision.messages)) {
        // additionalContext is injected once before the next model call —
        // append a user message to the pre-step's claimed messages.
        return {
          ...decision,
          messages: [...decision.messages, createTextUserMessage(context.join('\n\n'), 'dsh-hooks')],
        }
      }
      return decision
    } catch (error) {
      log(ctx, 'error', `agent ${agent.id} PostToolBatch hooks failed: ${String(error)}`)
      return next()
    }
  })
  ...
  agentCtx.on('agent/inbox/inserted', (payload) => {
    ...
    try {
      const hookInput = {
        ...hookInputBase(ctx, agent, 'UserPromptSubmit'),
  ...
  agentCtx.on('agent/session-start', (payload) => {
    ...
    const hookInput = {
      ...hookInputBase(ctx, agent, 'SessionStart'),
      source,
    }
    enqueue(state, async () => {
  ...
  // Stop: carries stop_hook_active (a previous Stop hook in THIS turn already
  // kept the turn alive) and last_assistant_message. A `decision: block` or
  // additionalContext keeps the turn going by injecting the reason as a
  // user message, mirroring CC's Stop semantics.
  //
  // The harness AWAITS the agent/turn-stopping serial dispatch before the turn
  // is considered finished, so this listener returns the queue promise: Stop
  // hooks complete before the turn ends (critical for one-shot headless runs,
  // where a fire-and-forget queue would be cut off by process exit).
  agentCtx.on('agent/turn-stopping', (payload) => {
    if (state.disposed) return
    const lastMessage = lastAssistantMessageText(agent.session ? agent.session.events : undefined)
    const hookInput = {
      ...hookInputBase(ctx, agent, 'Stop'),
      stop_hook_active: state.stopContinued,
      ...(lastMessage !== undefined ? { last_assistant_message: lastMessage } : {}),
    }
    return enqueue(state, async () => {
      const results = await executeHooks(ctx, state, agent, 'Stop', hookInput, payload && payload.signal)
      const continued = results.some(wantsContinue)
      state.stopContinued = continued
      if (continued) {
        // keep the turn going: inject the reason / context as a user message
        const reasons = results
          .filter((r) => !r.async && r.json)
          .map((r) => (r.json.decision === 'block' && typeof r.json.reason === 'string' ? r.json.reason : undefined))
          .filter((r) => r !== undefined)
        const context = collectAdditionalContext(results)
        const text = [reasons.join('\n'), context.join('\n\n')].filter((t) => t.length > 0).join('\n\n')
        if (text.length > 0) {
          try { agent.inject(createTextUserMessage(text, 'dsh-hooks')) } catch { /* best effort */ }
        }
      } else {
        state.stopContinued = false
      }
    }).catch(() => {})
  })

  // StopFailure: the turn ended on an error (LLM/API failure etc.). CC input
  // is base fields; we add `error` as a documented DSH extension.
  agentCtx.on('agent/error', (payload) => {
    if (state.disposed) return
    const hookInput = {
      ...hookInputBase(ctx, agent, 'StopFailure'),
      ...(payload && payload.error ? { error: { message: String((payload.error && payload.error.message) || payload.error) } } : {}),
    }
    enqueue(state, () => executeHooks(ctx, state, agent, 'StopFailure', hookInput, payload && payload.signal)).catch(() => {})
  })
}
...
export function apply(ctx) {
  ...
  ctx.on('agent/disposed', (payload) => {
    ...
    if (state !== undefined) {
      ...
      // CC SessionEnd input carries reason: 'exit'.
      const hookInput = {
        ...hookInputBase(ctx, agent, 'SessionEnd'),
        reason: 'exit',
      }
  ...
  // Subagent lifecycle events deliver the single `info` argument to cordis
  // listeners (the `parent` argument is not exposed). At subagent/start the
  // child is still registered, so we read its durable lineage
  // (session.header.parentSession) and cache the pairing; at subagent/end the
  // child may already be disposed, so we route through the cached map to find
  // the delegator's controller. Both SubagentStart and SubagentEnd fire AT
  // THE DELEGATOR, with agent_id/agent_type naming the child.
  ctx.on('subagent/start', (info) => {
    ...
    try {
      ...
      const hookInput = {
        ...hookInputBase(ctx, state.agent, 'SubagentStart'),
        agent_id: String(childId),
        agent_type: (info && info.provider) || 'subagent',
      }
      enqueue(state, () => executeHooks(ctx, state, state.agent, 'SubagentStart', hookInput)).catch(() => {})
    } catch (error) {
  ...
  ctx.on('subagent/end', (info) => {
    ...
    const hookInput = {
      ...hookInputBase(ctx, state.agent, 'SubagentEnd'),
      agent_id: String(childId),
      agent_type: (info && info.provider) || 'subagent',
      ...(info && typeof info.lastAssistantMessage === 'string'
        ? { last_assistant_message: info.lastAssistantMessage }
        : {}),
    }
    enqueue(state, () => executeHooks(ctx, state, state.agent, 'SubagentEnd', hookInput)).catch(() => {})
  })

  // Client console data channel (optional): HTTP route on the web carrier —
  // registered only when a webServer service is present (web profiles). The
  // floating console polls /dsh-hooks/recent. Headless/TUI profiles simply
  // skip this route.
  {
    ...
    if (webServer && typeof webServer.register === 'function') {
      ...
      log(ctx, 'info', 'no webServer service — web console route skipped')
    }
  }

  // Plugin unload / process exit: fire SessionEnd (reason 'exit') for any
  // still-live controller (headless one-shots never emit agent/disposed),
  // then tear down. In the web profile the per-agent agent/disposed handler
  // already runs SessionEnd and removes the state, so this only catches the
  // remaining ones.
  ctx.effect(() => () => {
    for (const state of [...agentStates.values()]) {
      if (state.disposed) continue
      const hookInput = {
        ...hookInputBase(ctx, state.agent, 'SessionEnd'),
        reason: 'exit',
      }
      enqueue(state, () => executeHooks(ctx, state, state.agent, 'SessionEnd', hookInput))
        .catch(() => {})
        .finally(() => cleanupState(ctx, state, 'plugin reloaded'))
    }
  })

  log(ctx, 'info', 'plugin active (v2) — per-agent hooks, CC protocol, rc.7 wiring')
}
